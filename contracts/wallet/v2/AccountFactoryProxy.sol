...
contract AccountFactoryProxy is AccountFactoryStorageBase {
    ...
    event ImplementationReset(address indexed oldImpl, address indexed newImpl);

    constructor(address impl, address owner, address walletTemplate) {
        implementation = impl;
        owner = msg.sender;
    ...
    function setImplementation(address impl) external onlyOwner {
        ...
        implementation = impl;
        emit ImplementationReset(oldImpl, impl);
    }

    receive() external payable {
        revert("do not ransfer to me");
    }
