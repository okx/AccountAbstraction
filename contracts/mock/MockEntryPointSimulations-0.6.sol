...
import "../@eth-infinitism-v0.6/interfaces/IAccount.sol";
import "../@eth-infinitism-v0.6/interfaces/IPaymaster.sol";
import "../@eth-infinitism-v0.6/interfaces/IEntryPoint.sol";
import "../@eth-infinitism-v0.6/utils/Exec.sol";
import "../@eth-infinitism-v0.6/core/EntryPoint.sol";
import "../@eth-infinitism-v0.6/core/StakeManager.sol";
import "../@eth-infinitism-v0.6/core/SenderCreator.sol";
import "../@eth-infinitism-v0.6/core/Helpers.sol";
import "../@eth-infinitism-v0.6/core/NonceManager.sol";
import "@openzeppelin/contracts/security/ReentrancyGuard.sol";
import "hardhat/console.sol";

contract MockEntryPointSimulations is
    IEntryPoint,
    StakeManager,
    NonceManager,
    ReentrancyGuard
{
    ...
    function syncSenderAndePaymasterDeposits(UserOperation calldata op) public {
        syncDeposit(op.sender);
        bytes calldata paymasterAndData = op.paymasterAndData;
        if (paymasterAndData.length >= 20) {
            syncDeposit(address(bytes20(paymasterAndData[:20])));
        }
    }

    function syncDeposit(address target) public {
        deposits[target].deposit = uint112(ENTRYPOINT.balanceOf(target));
    }

    function syncNonce(address target, uint256 nonce) public {
    ...
    function simulateHandleOp(
        ...
        syncSenderAndePaymasterDeposits(op);
        syncNonce(op.sender, op.nonce);
        ...
        revert ExecutionResultCostom(
            opInfo.preOpGas,
            mode,
            excuteResult,
            gasPrice > 0 ? paid / gasPrice : 0,
            callGasCost,
            gasPrice,
            data.validAfter,
            data.validUntil,
        ...
        )
    {
        ...
        if (callData.length > 0) {
            ...
            excuteResult = Exec.getReturnData(REVERT_REASON_MAX_LEN);
            if (!success) {
                if (excuteResult.length > 0) {
