...
describe("SmartAccount", function () {
    ...
    describe("syncNonce", function () {
        it("should sync target's nonce", async function () {
            ...
            await mockEntryPointSimulation0_6.connect(owner).syncNonce(owner.address, 0)

            await expect(await mockEntryPointSimulation0_6.getNonce(owner.address, 0)).to.equal(3);
        });
    ...
    describe("syncDeposit", function () {
        it("should sync target's deposit", async function () {
            ...
            await entrypoint0_6.connect(owner).depositTo(owner.address, { value: ethers.utils.parseEther("1") })

            await expect(await entrypoint0_6.balanceOf(owner.address)).to.equal(ethers.utils.parseEther("1"));
            await expect(await mockEntryPointSimulation0_6.balanceOf(owner.address)).to.equal(ethers.utils.parseEther("0"));



            await mockEntryPointSimulation0_6.connect(owner).syncDeposit(owner.address)

            await expect(await mockEntryPointSimulation0_6.balanceOf(owner.address)).to.equal(ethers.utils.parseEther("1"));
    ...
    describe("syncSenderAndePaymasterDeposits", function () {
        it("should sync sender and paymaster's deposits", async function () {
            ...
            await entrypoint0_6.connect(owner).depositTo(aliceProxyAccount.address, { value: ethers.utils.parseEther("1") })
            await entrypoint0_6.connect(owner).depositTo(TokenPaymaster.address, { value: ethers.utils.parseEther("1") })

            await expect(await entrypoint0_6.balanceOf(aliceProxyAccount.address)).to.equal(ethers.utils.parseEther("1"));
            await expect(await entrypoint0_6.balanceOf(TokenPaymaster.address)).to.equal(ethers.utils.parseEther("1"));
            await expect(await mockEntryPointSimulation0_6.balanceOf(aliceProxyAccount.address)).to.equal(ethers.utils.parseEther("0"));
            await expect(await mockEntryPointSimulation0_6.balanceOf(TokenPaymaster.address)).to.equal(ethers.utils.parseEther("0"));

            await mockEntryPointSimulation0_6.syncSenderAndePaymasterDeposits(userOp)


            await expect(await mockEntryPointSimulation0_6.balanceOf(aliceProxyAccount.address)).to.equal(ethers.utils.parseEther("1"));
            await expect(await mockEntryPointSimulation0_6.balanceOf(TokenPaymaster.address)).to.equal(ethers.utils.parseEther("1"));
    ...
    describe("simulateHandleOps", function () {
        it("Should correctly revert call revert reason", async function () {
            ...
            await expect(parsedError.args.result
                .includes(hex)).to.equal(true);
        ...
        it("Should correctly revert gas Limit", async function () {
            ...
            userOp = await Utils.generateSignedUOPWithManualGasLimit({
                ...
                sigType: 1,
                sigTime: 0,
                manualVerificationGasLimit: (parsedError.args.preOpGas.sub(parsedError.args.callGasCost)).mul(k * 10).div(10),
                manualPreVerificationGas: 0,
                manualCallGasLimit: parsedError.args.callGasCost.mul(k * 10).div(10)
            });

            let tx = await entrypoint0_6.connect(bundler).handleOps([userOp], owner.address);
