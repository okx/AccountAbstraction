...
describe("TradeJoeV2Adapter", function () {
  async function deploy() {
    ...
    let MockTradeJoeV2RouterFactory = await ethers.getContractFactory(
      "contracts/mock/MockTradeJoeV2.sol:MockTradeJoeV2Router"
    );
    let MockTradeJoeV2Router = await MockTradeJoeV2RouterFactory.deploy(
      WNativeToken
    );
    ...
    let SwapHelper = await SwapHelperFactory.deploy(
      MockTradeJoeV2Router.address, owner.address
    );
  ...
  describe("setAirBinStep", function () {
    it("should setAirBinStep correctly", async function () {
      const {
        EntryPoint,
        signer,
        owner,
      ...
      const airBinStep = [ethers.BigNumber.from(0)];

      await SwapHelper.setAirBinStep(TestToken.address, airBinStep);

      for (let i = 0; i < airBinStep.length; i++) {
        expect(airBinStep[i]).to.equal(
          await SwapHelper.airBinSteps(TestToken.address, i)
        );
      }
    });
    ...
    it("should emit event", async function () {
      ...
      const setAirBinStepTx = await SwapHelper.setAirBinStep(
        TestToken.address,
        airBinStep
      );
      const receipt = await setAirBinStepTx.wait();
      const event = receipt.events[0];

      expect(event.event).to.equal("AirBinStepSet");
      expect(event.args.token).to.equal(TestToken.address);
      expect(event.args.airBinStep).to.eql(airBinStep); // note: "eql" does a deep equality comparison
    ...
    it("should revert if the caller is not owner", async function () {
      ...
      await expect(
        SwapHelper.connect(signer).setAirBinStep(TestToken.address, airBinStep)
      ).to.be.revertedWith("Ownable: caller is not the owner");
    });
  });
  ...
  describe("swapToNative", function () {
    it("should revert With SwapHelper: path not found ", async function () {
      ...
      await owner.sendTransaction({
        to: MockTradeJoeV2Router.address,
    ...
    it("should revert With SwapHelper: path not found ", async function () {
      ...
      ).to.be.revertedWith("SwapHelper: airBinStep not found");
    });
