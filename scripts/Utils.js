...
async function deploy(DeployFactory, contractName, constructorInfor, salt) {
  const contract = await ethers.getContractFactory(contractName);

  const initCode = ethers.utils.solidityPack(
    ["bytes", "bytes"],
    [ethers.utils.hexDataSlice(contract.bytecode, 0), constructorInfor]
  );

  await DeployFactory.deploy(initCode, salt);
  console.log(await DeployFactory.getAddress(initCode, salt));

  return await DeployFactory.getAddress(initCode, salt);
}

async function getDeployedAdderss(
  DeployFactory,
  contractName,
  constructorInfor,
  salt
